  ...
  "Did anything in that paragraph seem off to you? Maybe not. The grammar is perfect, the tone works, and the narrative flows.\n";
    ...
    #pragma omp parallel
    {
        ...
        if (!curl) {
            ...
            exit(1); // Handle initialization failure more gracefully as needed
        }

        #pragma omp for reduction(+:total_similarity) reduction(+:comparisons)
    ...
    if (comparisons > 0) {
        double average_similarity = (total_similarity / comparisons) * 100;
